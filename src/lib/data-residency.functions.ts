...
const ConfigInput = z.object({
  ...
  supervisor_telegram_chat_id: z.string().max(64).nullable().optional(),
...
export const saveResidencyConfigFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const row = {
      ...
      ...(data.supervisor_telegram_chat_id !== undefined ? { supervisor_telegram_chat_id: data.supervisor_telegram_chat_id ?? null } : {}),
      ...(typeof data.notify_on_escalation === "boolean" ? { notify_on_escalation: data.notify_on_escalation } : {}),
    };
