...
function CallsPage() {
  ...
  useEffect(() => {
    supabase
      .from("calls")
      .select("id,twilio_call_sid,direction,from_number,to_number,status,duration_seconds,started_at,created_at,recording_path,recording_url,transcript,summary")
  ...
  const exportCsv = () => {
    const header = ["created_at", "direction", "from", "to", "status", "duration_seconds", "summary"];
    const rows = filtered.map((c) => [c.created_at, c.direction, c.from_number ?? "", c.to_number ?? "", c.status, String(c.duration_seconds), (c.summary ?? "").replace(/\n/g, " ")]);
  ...
  const togglePlay = async (c: Call) => {
    ...
    try {
      ...
      console.error("[calls] recording url failed", e);
    } finally {
      ...
      <div className="flex flex-col sm:flex-row gap-2 mb-4">
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder={lang === "ru" ? "Поиск по номеру или тексту звонка…" : lang === "ro" ? "Caută după număr sau text…" : "Search by number or transcript…"}
            className="pl-9 pr-9"
          />
          {q && (
        ...
        <Select value={direction} onValueChange={setDirection}>
          ...
          <SelectContent>
            <SelectItem value="all">{lang === "ru" ? "Все направления" : lang === "ro" ? "Toate" : "All directions"}</SelectItem>
            <SelectItem value="inbound">{lang === "ru" ? "Входящие" : lang === "ro" ? "Intrare" : "Inbound"}</SelectItem>
            <SelectItem value="outbound">{lang === "ru" ? "Исходящие" : lang === "ro" ? "Ieșire" : "Outbound"}</SelectItem>
          </SelectContent>
        </Select>
        <Select value={status} onValueChange={setStatus}>
          <SelectTrigger className="w-full sm:w-[160px]"><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">{lang === "ru" ? "Все статусы" : lang === "ro" ? "Toate stările" : "All statuses"}</SelectItem>
        ...
        <Select value={sort} onValueChange={(v) => setSort(v as SortKey)}>
          ...
          <SelectContent>
            <SelectItem value="date_desc">{lang === "ru" ? "Сначала новые" : lang === "ro" ? "Cele mai noi" : "Newest first"}</SelectItem>
            <SelectItem value="date_asc">{lang === "ru" ? "Сначала старые" : lang === "ro" ? "Cele mai vechi" : "Oldest first"}</SelectItem>
            <SelectItem value="duration_desc">{lang === "ru" ? "Дольше первыми" : lang === "ro" ? "Cele mai lungi" : "Longest first"}</SelectItem>
            <SelectItem value="duration_asc">{lang === "ru" ? "Короче первыми" : lang === "ro" ? "Cele mai scurte" : "Shortest first"}</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      {loading ? (
        ...
        <Card className="bg-gradient-card border-dashed border-2">
          <CardContent className="py-12 text-center text-sm text-muted-foreground">
            {lang === "ru" ? "Ничего не найдено" : lang === "ro" ? "Nimic găsit" : "No results"}
          </CardContent>
        </Card>
      ) : (
                  ...
                  return (
                    <Card key={c.id} className="bg-gradient-card shadow-soft hover:shadow-elegant transition-shadow">
                      <CardContent className="p-4">
                        <div className="flex items-center gap-3 sm:gap-4">
                          ...
                          {hasRecording && (
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={(e) => { e.preventDefault(); togglePlay(c); }}
                        ...
                        {snippet && (
                          <div className="mt-2 ml-12 text-xs text-muted-foreground italic">
                            …{highlight(snippet)}…
                          </div>
                        )}
